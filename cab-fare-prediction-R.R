...
setwd('I:/Projects/cab-fare-prediction-R')

# #loading Libraries
x = c("ggplot2", "corrgram", "DMwR", "usdm", "caret", "randomForest", "e1071", "unbalanced","C50", "dummies", "Information",
...
# # Number of attributes in the dataset are as follows:
...
# Fare amount can't negative value & it cannot be a '0', So removing them,
...
# Since there are 20 observations in which passengers travelled in cab are more than 6(which is not possible), so removing them
train[which(train$passenger_count > 6 ),]
# Also passengers trvelled can't be less than 1
train[which(train$passenger_count <1 ),]
nrow(train[which(train$passenger_count <1 ),])
# Removing these 58 observations and 20 observation which are above 6 value because a cab cannot hold these number of passengers.
...
# Calculate the haversine distance using longitude and latitude
...
ggcorrplot::ggcorrplot(corr, method=("square"),type=typtrain[,numeric_index],upper.panel=panel.pie, title =  "Correlation Plot")
...
# dev.off()
par(mfrow=c(1,2))
ggplot2(train$fare_amount)                         # qqPlot, it has a x values derived from gaussian distribution, if data is distributed normally then the sorted data points should lie very close to the solid reference line 
...
xgboost_model2 = xgboost(data = train_data_matrix2,label = train$fare_amount,nrounds = 25,verbose = FALSE)

# Saving the trained model
saveRDS(xgboost_model2, "./final_Xgboost_model_using_R.rds")

# loading the saved model
best_model <- readRDS("./final_Xgboost_model_using_R.rds")
print(best_model)

# Lets now predict on test dataset
xgb = predict(best_model,test_data_matrix2)
